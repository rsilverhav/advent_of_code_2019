...
fn get_wire_locations(input: &String) -> HashSet<Position> {
    let mut position = Position::new(0, 0);
    let mut wire_locations: HashSet<Position> = HashSet::new();
    for instruction in input.split(",") {
        ...
        let delta = match direction {
            ...
            "U" => Position::new(0, 1),
            "D" => Position::new(0, -1),
            _ => panic!("Missing instruction {}", direction),
        };
        for _ in 0..*length {
            position.add(&delta);
            wire_locations.insert(position.clone());
        }
    }
    wire_locations
}

fn get_closest_intersection(wire_1: &String, wire_2: &String) -> Position {
    let result_wire_1 = get_wire_locations(wire_1);
    let result_wire_2 = get_wire_locations(wire_2);

    let mut intersections: Vec<Position> = Vec::new();
    for wire_location in result_wire_1 {
        if result_wire_2.contains(&wire_location) {
            intersections.push(wire_location);
        }
    }
    intersections.sort();
    intersections.first().unwrap().clone()
}

fn get_closest_walking_intersection(wire_1: &String, wire_2: &String) -> i32 {
    let result_wire_1 = get_wire_locations(wire_1);
    let result_wire_2 = get_wire_locations(wire_2);

    let mut intersections: Vec<(Position, Position)> = Vec::new();
    for wire_location_1 in result_wire_1 {
        if result_wire_2.contains(&wire_location_1) {
            intersections.push((
                wire_location_1.clone(),
                result_wire_2.get(&wire_location_1).unwrap().clone(),
            ));
        }
    }
    intersections.sort_by(|a, b| {
        (a.0.distance + a.1.distance)
            .partial_cmp(&(b.0.distance + b.1.distance))
            .unwrap()
    });
    let nearest_intersection = intersections.first().unwrap();
    nearest_intersection.0.distance + nearest_intersection.1.distance
}

fn part_1(input_wire_1: &String, input_wire_2: &String) {
    assert_eq!(
        get_closest_intersection(&String::from("R8,U5,L5,D3"), &String::from("U7,R6,D4,L4"))
            .distance_from_origo(),
        6
    );
    assert_eq!(
    ...
    println!(
        "Part 1 result: {:?}",
        get_closest_intersection(input_wire_1, input_wire_2).distance_from_origo()
    )
}

fn part_2(input_wire_1: &String, input_wire_2: &String) {
    assert_eq!(
        get_closest_walking_intersection(
